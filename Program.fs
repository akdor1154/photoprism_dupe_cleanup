...
let list_dir (dir:string) =
    ...
    let files =
        ...
        |> Seq.map (fun (fn, path, m) -> {
            ...
            sidecar =
                let sidecar_path = System.IO.Path.GetFileNameWithoutExtension(path) + ".json" in
        ...
        for (dupe_candidate, dupe_hash) in hashes do
            let dupes_of = hashes |> List.choose (fun (other_candidate, other_hash) ->
                if dupe_candidate = other_candidate then None
                    ...
                    if similarity > 98.0 && dupe_size < other_size/4L then
                        Some({Dupe.small_dupe = dupe_candidate; other = other_candidate; similarity=similarity})
                    else None
            ) in
            match dupes_of with
            |[d] -> Some(d)
            ...
            |many ->
                let sims = many
                    ...
                    |> Seq.map (sprintf "%.2f")
                    |> String.concat "," in
                printfn "warning: got multiple dupe candidates for %s.\n%s" dupe_candidate.name sims;
                None
        done
    }
    |> Seq.choose (id)
...
let main argv =
    ...
    let dupes = Seq.collect (fun (ext, files) -> get_dupes ext files ) sets in

    for d in dupes do
        let MB i = float(i)*10E-6 in
        printfn "Found a dupe:";
        printfn "%s, %.1fM, probs dupe of" d.small_dupe.name (MB d.small_dupe.size);
        printfn "%s, %.1fM." d.other.name (MB d.other.size);
    done;
